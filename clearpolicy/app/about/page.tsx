    ...
    <div className="grid grid-cols-1 gap-6">
      ...
      <Card>
        ...
        <ol className="mt-3 grid gap-4 md:grid-cols-3 text-sm">
          <li className="rounded-lg border border-[var(--cp-border)] bg-[var(--cp-surface-2)] p-4">
          ...
          <ul className="flex gap-3 min-w-full pr-2">
            {quickLinks.map((l) => (
              <li key={l.slug} className="min-w-[16rem]">
                <Link href={`/measure/${l.slug}`} className="block rounded-lg border border-[var(--cp-border)] bg-[var(--cp-surface)] p-4 focus-ring">
